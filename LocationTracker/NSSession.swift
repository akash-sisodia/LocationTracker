...
//  NSSession.swift
//
//  * Supporting swift version 3.1 and 5 *
//
...
//  - SSL-pinning
//
//  - Copyright (c) 2017 Akash Singh Sisodia. All rights reserved.
//
...
class NSSession: NSObject {
    ...
    // MARK: Public
    public func requestWith(path: String, method: HTTPMethod, parameters: HTTPParameters?, retryCount:Int, shouldCacheResponse: Bool = true, completionHandler: @escaping(Bool, Data?, Error?) -> Void) -> Void {
        ...
        do {
            if let json = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
                ...
                if let success = json["success"] as? Bool, success == true {
                    ...
                    if let error = json["error"] as? HTTPParameters, let message = error["message"] as? String {
